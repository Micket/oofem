    ...
    for ( int i = 0; i < this->total; ++i ) {
        ...
        if ( rp ) {
            if ( rp->subgrid ) {
                ...
                OOFEM_ERROR ( "ParticleGrid<Point> :: appendAllPoints - Refined particle does not contain subgrid or point." );
            }
        }
    }
}
...
public:
    ...
    void setPoint ( Point *p ) {
        if ( this->end() ) {
            OOFEM_ERROR ( "ParticleGrid - Can't set element in outside grid" );
        }
        if ( this->subGridActive() ) {
